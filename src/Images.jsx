...
class Images extends React.Component {
    ...
    handleRemoveImage() {
        ...
        utils.podmanAction("RemoveImage", { name: image }, this.state.imageWillDelete.isRoot)
                .catch(ex => {
    ...
    handleForceRemoveImage() {
        ...
        utils.podmanAction("RemoveImage", { name: id, force: true }, this.state.imageWillDelete.isRoot)
                .then(reply => {
                    this.setState({
                        setImageRemoveErrorModal: false
                    });
    ...
    renderRow(image) {
        ...
        let columns = [
            ...
            image.isRoot ? _("system") : this.props.user,
            {
                element: runImage,
                tight: true
            }
        ...
        tabs.push({
            ...
            data: {
                containers: this.props.imageContainerList !== null ? this.props.imageContainerList[image.id + image.isRoot.toString()] : null,
        ...
        return (
            <Listing.ListingRow
                    key={image.id + image.isRoot.toString()}
                    rowId={image.id + image.isRoot.toString()}
                    columns={columns}
                    tabRenderers={tabs}
    ...
    render() {
        const columnTitles = [ _("Name"), _(''), _("Created"), _("Size"), _("Owner"), _('') ];
