...
class Containers extends React.Component {
    ...
    stopContainer(container, force) {
        ...
        if (force)
            args.timeout = 0;
        utils.podmanAction("StopContainer", args, container.isRoot)
    ...
    startContainer(container) {
        utils.podmanAction("StartContainer", { name: container.names }, container.isRoot)
                .catch(ex => this.setState({
    ...
    restartContainer (container, force) {
        ...
        if (force)
            args.timeout = 0;
        utils.podmanAction("RestartContainer", args, container.isRoot)
                .catch(ex => this.setState({
    ...
    renderRow(containersStats, container) {
        const containerStats = containersStats[container.id + container.isRoot.toString()];
        ...
        let mem = "";
        if (containerStats) {
            proc = containerStats.cpu ? utils.format_cpu_percent(containerStats.cpu * 100) : <abbr title="not available">n/a</abbr>;
            mem = containerStats.mem_usage ? utils.format_memory_and_limit(containerStats.mem_usage, containerStats.mem_limit) : <abbr title="not available">n/a</abbr>;
        }
        let columns = [
            ...
            proc,
            mem,
            container.isRoot ? _("system") : this.props.user,
        ...
        let tabs = [{
            ...
            data: { containerId: container.id, containerStatus: container.status, width:this.state.width, root:container.isRoot }
        }];
        ...
        return (
            <ScrollableAnchor id={container.id} key={container.id}>
                <Listing.ListingRow
                        key={container.id + container.isRoot.toString()}
                        rowId={container.id + container.isRoot.toString()}
                        columns={columns}
                        tabRenderers={tabs}
    ...
    handleRemoveContainer() {
        ...
        utils.podmanAction("RemoveContainer", { name: id }, this.state.containerWillDelete.isRoot)
    ...
    handleForceRemoveContainer() {
        const id = this.state.containerWillDelete ? this.state.containerWillDelete.id : "";
        utils.podmanAction("RemoveContainer", { name: id, force: true }, this.state.containerWillDelete.isRoot)
                .then(reply => {
                    this.setState({
                        setContainerRemoveErrorModal: false
                    });
